...
class Assistant:
    def __init__(self, name: str):
        ...
        model_args.max_history = 2000
        self.model = ConvAIModel(model_type="gpt", model_name="data/gpt_personachat_cache", args=model_args)
    ...
    @staticmethod
    def search_anything_google(command: str):
...
def main(speech: bool):
    ...
    def cmd_handler(cmd: str):
        ...
        if re.search('date', cmd):
            ...
            news_result = cy.news()
            cy.speak("Source: The Times of India")
        ...
        elif "goodbye" in cmd or "offline" in cmd or "bye" in cmd or "exit" in cmd:
    ...
    if speech:
        while True:
            ...
            if cmd_handler(cmd=cmd):
                cy.text, cy.history = cy.model.interact_single(
                    cmd,
                    cy.history,
                    cy.persona
                )
