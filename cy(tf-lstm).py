...
model_path = '/home/pi/Desktop/Cy/data/tf-lstm/model.h5'
...
class Assistant:
    def __init__(self, name: str):
        ...
        with open('data/tf-lstm/intents.json') as intents:
        ...
        intents = pd.read_csv("data/tf-lstm/intents_processed.csv")
        self.tokenizer = Tokenizer(num_words=2000)
        self.tokenizer.fit_on_texts(intents['patterns'])
        self.label_encoder = LabelEncoder()
        self.label_encoder.fit_transform(intents['tags'])
...
def main(speech: bool):
    ...
    def cmd_handler(cmd: str):
        ...
        if re.search('date', cmd):
            ...
            topic = cmd.replace("tell me about ","")
            #topic = cmd.split(' ')[-2]+" "+cmd.split(' ')[-1]
            if topic:
                #print(topic)
    ...
    if speech:
        while True:
            ...
            if cmd_handler(cmd=cmd):
                ...
                cmd = [letters.lower() for letters in cmd if
                       letters not in string.punctuation]
                cmd = ''.join(cmd)
                texts_p.append(cmd)
                # Tokenizing and padding
                cmd = cy.tokenizer.texts_to_sequences(texts_p)
                cmd = np.array(cmd).reshape(-1)
                cmd = pad_sequences([cmd], 7)
                # Getting output from model
                output = cy.model.predict(cmd)
                output = output.argmax()
                # Finding the right tag and predicting
                response_tag = cy.label_encoder.inverse_transform([output])[0]
                cy.text = random.choice(cy.responses[response_tag])
                cy.speak(cy.text)
